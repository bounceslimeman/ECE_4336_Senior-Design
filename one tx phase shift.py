...
num_scans = 50                                 # Number of scanning iterations.
...
phase_shift = np.pi * (float(desiredPhaseShift) / np.pi )
...
for i in range(num_scans):  # This block performs multiple scans, adjusting the phase shift between Rx_0 and Rx_1, summing the signals, and finding the peak.
    ...
    if Plot_Compass == False:
        ...
        std_dev_max_gain = np.std(max_gains)  # Calculate the standard deviation
    
        # Display both max gain, average max gain, and standard deviation
        plt.text(140, 0, f"Gain = {max_gain:.1f} dB\nAvg Gain = {avg_max_gain:.1f} dB\nStd Dev = {std_dev_max_gain:.1f} dB",
